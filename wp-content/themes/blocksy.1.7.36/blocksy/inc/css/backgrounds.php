	...
	function blocksy_output_single_background_css($args = []) {
		...
		if ($args['value']['background_type'] === 'image') {
			...
			if (! empty($attachment_url)) {
				...
				$args['css']->put(
					$args['selector'],
					"--backgroundPosition: " . (floatval($background_image['x']) * 100) . "% " . (
						floatval($background_image['y']) * 100
					) . '%'
				);

				$background_size = blocksy_akg('background_size', $args['value'], 'auto');
				...
				$background_attachment = blocksy_akg(
				...
				$background_repeat = blocksy_akg('background_repeat', $args['value'], 'no-repeat');
