...
import { ColorPickerContext } from '../options/ct-color-picker'
...
const SingleChoice = ({
	...
	return (
		<ColorPickerContext.Provider value={{ modalWrapper: ref }}>
			<div key={singleChoice.id} className="ct-labeled-group-item">
				...
				{groupOption.options[singleChoice.id].type ===
					'ct-background' && (
					<div
				...
				{['ct-color-picker', 'ct-border', 'ct-box-shadow'].includes(
					groupOption.options[singleChoice.id].type
				) && <div ref={ref} className="ct-color-modal-wrapper" />}
			</div>
...
const LabeledGroup = ({
	...
	return renderingChunk.map(groupOption => {
		...
		const totalAmountofMatched = groupOption.choices.filter(singleChoice =>
